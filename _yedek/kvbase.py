...
class KVBase():
        ...
        #create dict[pidm, name] from data[pidm]
                try:
                        dict = []
                        # dumps = json.dumps(data) #önce obj->stringe çevir
                        # # 2 saatime mal oldu bunu bulmak
                        # if (dumps=="[]"): return [] #kv_pidmleri silince ve bişey kalmadığında çıkan hatayı kontrol etmek için.
                        # loads = json.loads(dumps) #sonra str->objeye

                        records = array('i', data)

                        for item in records:
                                table_pidm = item
                                table_name = self.getPidmName(tableName, table_pidm )
                                dict.append({tableName+'_pidm':table_pidm,tableName+'_name':table_name})

                        # _json = jsonify(dict)  #json'a çevirme, dict olarak dönmesi lazım
